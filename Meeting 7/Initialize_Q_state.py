...
class Oracle(): #construct quantum state for data and label
    def __init__(self, data ,label, **kwargs):
        ...
        for i in data:
            ...
            self.state = 1/norm*sum([i[k-1]*qt.fock(self.N+1, k) for k in range(1,self.N+1)])
        ...
        self.qlabels = 1/(np.linalg.norm(label))*sum([label[i-1]*qt.fock(self.M+1,i) for i in range(1,self.M+1)])
        
        #construct oracle operator for training
        self.Train_Oracle = sum([qt.tensor( self.norms[i-1]*self.qstates[i-1]*qt.fock(self.M+1, 0).dag(),qt.fock(self.M+1, i)*qt.fock(self.M+1, i).dag() ) for i in range(1, self.M+1)])
...
def kernel(train_oracle ,qstat,m, n): #m is number of datasets, n the length of data vector
    ...
    chi = train_oracle*sum([qt.tensor(qt.fock(m+1,0),qt.fock(m+1,i)) for i in range(1,m+1)])
