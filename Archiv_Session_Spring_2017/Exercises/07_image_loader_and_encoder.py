...
def prep_datas(xset, xlabels):
    ...
    for i in range(len(X)):
        # reduce the size of the image from 100X100 to 32X32. Also flattens the
        # color levels
        X[i] = resize(X[i], (32, 32, 1))
...
# reshape vectors again to images
decoded_imgs = [np.reshape(x,  (32, 32)) for x in decoded_imgs]
original_imgs = [np.reshape(x, (32, 32))
                 for x in X]  # reshape vectors again to images
...
for k in range(length):
    ...
    for i in range(outputdim):
        list.append(
            sum([x for x in encoded[i:(i + int(encoding_dim / outputdim))]]))
    output.append(list)
print(output)
